...
import { useFindUserOrdersQuery } from '@/shared/api/ordersApi';

function OrderPage() {
  const { data: orders } = useFindUserOrdersQuery();
    ...
    <>
      <h1
        className={cn(
          ...
          orders?.content.length === 0 && 'text-center'
        )}
      >
        Order History
      </h1>
      {orders && orders.content.length > 0 ? (
        <OrderTabs orders={orders.content} />
      ) : (
