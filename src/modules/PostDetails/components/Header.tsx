...
export const Header = ({ editor }: Props) => {
    const { form, defaultValues } = useContext(FormContext);
    ...
    const checkIsFormDirty = () => {
        ...
        const isContentSame = editorText === defaultValues.content;
    ...
    const handleSave = async () => {
        try {
            const { id, author, content, image, title } = form.values;
            await dispatch({ id: id.toString(), authorName: author, content, title, posterImgSrc: image }).unwrap();
            setIsEdited(false);
        } catch (err) {
            console.log(err);
        }
