                ...
                {status !== "DRAFT" && <div className="flex flex-col items-start gap-4 px-4 py-6">
                    {status === "PUBLISHED" && (
                        <>
                            ...
                            <ArchivePostModal id={Number(form.values.id)} setNotification={setNotification_archived} customHandler={(openModal) => <UnstyledButton onClick={openModal} className="inline-flex gap-2 items-center text-black text-sm outline-none p-0"><FolderDown size={16} />Add to archive</UnstyledButton>}/>
                        </>
                    )}
                    <DeletePostModal redirect="/admin/blogs" id={Number(form.values.id)} setNotification={setNotification_deleted} customHandler={(openModal) => <UnstyledButton onClick={openModal} className="inline-flex gap-2 items-center text-black text-sm outline-none p-0"><Trash2 size={16} /> Delete article</UnstyledButton>} />
                </div>}
            </div>
