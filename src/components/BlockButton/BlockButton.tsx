...
export default function BlockButton({ onClick, children, ...props }: Props) {
    ...
    return (
        <>
            <UnstyledButton onClick={context && context.unsavedChanges ? openMain : onClick} {...props}>{children}</UnstyledButton>
            <WarnModal>
                {(Modal) => (
                    <>
                        <Modal
                            ...
                            footerProps={{
                                ...
                                secondaryBtnOnClick: () => {
                                    closeMain();
                                    context.update(prev => ({ ...prev, unsavedChanges: false }));
                                    onClick();
                                },
                                primaryBtnOnClick: closeMain,
                                primaryBtnText: 'Stay',
                                containerStyles: { display: 'flex', justifyContent: 'end', marginTop: "24px" },
                            }}
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <hgroup>
                                    ...
                                    <p className="text-sm text-[#A0A0A0]">Are you sure you want to proceed?</p>
                                </hgroup>
                            </div>
                        </Modal>
                    </>
